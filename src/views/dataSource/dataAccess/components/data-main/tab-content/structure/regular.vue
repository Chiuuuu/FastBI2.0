<template>
  ...
  >
    ...
    <a-spin :spinning="spinning">
      <a-form-model ref="form" :model="form" :rules="regRules" :label-col="{ span: 4 }" :wrapper-col="{ span: 19 }">
        <template v-if="single">
          ...
          >
            ...
            >
              <a-select-option v-for="item in increaseList[0].fieldList" :key="item.name" :value="item.name">
                {{ item.name }}
              </a-select-option>
            </a-select>
          </a-form-model-item>
        ...
        <a-form-model-item style="margin-left: 100px" prop="interval" v-if="form.repeat === 1" :rules="intervalRules">
          <div>
            <span>每隔&nbsp;</span>
      ...
      >
        <template #extractType="list, record, index">
          ...
          >
            ...
            <a-select-option
              :value="1"
              :disabled="tableType === 1 || isIncreasableTable(record) || isLargeData(record.target)"
            >
              增量更新
            </a-select-option>
          </a-select>
...
export default {
  ...
  methods: {
    ...
    isIncreasableTable(table) {
      // 新建任务的时候
      let target = null;
      if (!this.regData.length) {
        target = this.allTable.find(item => item.id === table.target);
      } else {
        target = this.regData.find(item => item.id === table.id);
      }
    ...
    async handleAsyncRegularInfo(type) {
      ...
      if (this.tableType === 0) {
        ...
        if (type === 'add') {
          return this.$server.dataAccess.addRegularInfo(paramList).finally(() => {
            this.loading = false;
          });
        } else if (type === 'update') {
          return this.$server.dataAccess.putRegularInfo(paramList).finally(() => {
            this.loading = false;
          });
        }
        ...
        if (type === 'add') {
          return this.$server.dataAccess.addCustomRegularInfo(paramList).finally(() => {
            this.loading = false;
          });
        } else if (type === 'update') {
          return this.$server.dataAccess.putCustomRegularInfo(paramList).finally(() => {
            this.loading = false;
          });
        }
      }
