...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        extractType: [{ required: true, message: '请选择抽取方式', trigger: 'change' }],
        id: [{ required: true, message: '请选择依据字段', trigger: 'change' }],
      },
      regData: [],
      regColumns,
    };
  ...
  methods: {
    ...
    async handleGetRegularList() {
      ...
      let res;
      if (!this.single) {
        res = await this.$server.dataAccess.getRegularList(this.databaseId, 1).finally(() => {
          this.modalSpin = false;
        });
      } else {
        res = await this.$server.dataAccess.getRegularList(this.rows[0].id, 0).finally(() => {
          this.modalSpin = false;
        });
      }
      if (res.code === 200) {
