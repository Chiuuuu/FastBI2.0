<template>
  <div class="popover-row" :class="error ? 'z-err' : ''">
    ...
    <div class="err-message">无法关联，字段数据类型不一致</div>
  </div>
</template>
<script>
export default {
  ...
  methods: {
    ...
    handlePopoverSelectChange(type, value, row) {
      const leftNode = this.popoverLeftList
        .filter(item => item.id === (type === 'left' ? value : row.leftFieldId))
        .pop();
      const rightNode = this.popoverRightList
        .filter(item => item.id === (type === 'left' ? row.rightFieldId : value))
        .pop();
