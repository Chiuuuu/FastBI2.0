...
export class TokenStream {
  ...
  lexDate() {
    ...
    if (escaped) {
      ...
      const matchs =
        /^ *(\d{4})([-/.])(0?\d|1[0-2])\2(0?\d|[12]\d|3[01])( +(0?\d|1\d|2[0-3]):(0?\d|[1-5]\d):(0?\d|[1-5]\d))? *$/.exec(
          subStr,
        );
      if (matchs) {
        let year = matchs[1];
        ...
        if (
          ...
          (this.isLeapYear(year) && (month === '2' || month === '02') && day === '29')
        ) {
          return {
            type: 'dateLit',
  ...
  lexString() {
    const { str, escaped } = this.skipSpace ? this.escapeSquare('"') : this.matchSquare('"');
    if (escaped) {
      return {
        type: 'string',
        value: str,
  ...
  lexNumber() {
    ...
    if (hasDot) {
      ...
      if (!this.isDigit(this.source.peek())) {
        return {
          type: 'integer',
          value: this.skipSpace ? parseInt(number) : number.substring(0, number.length - 1),
        };
      } else {
  ...
  lexIdentifier() {
    ...
    if (this.source.peek() === '-') {
      ...
      index++;
      let str = '';
      while (!(index >= this.source.length) && this.isLetter(this.source.peek(index))) {
