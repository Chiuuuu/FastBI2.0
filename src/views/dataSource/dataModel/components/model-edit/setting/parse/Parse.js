...
export class Parse {
  ...
  // 未知错误
  unexpected() {
    this.tokenStream.tokenRaiseError('Unexpected token: ' + JSON.stringify(this.tokenStream.tokenPeek()));
  }
  ...
  delimited(start, stop, separator, parse, func) {
    ...
    if (func && 'got' in func && args.length !== func.got) {
      throw new Error(`参数个数错误：方法${func.value}，应该符合${func.got}个参数，而不是${args.length}个`);
    }
    if (func && 'min' in func && args.length < func.min) {
      throw new Error(`参数个数错误：方法${func.value}，至少有${func.min}个参数，而不是${args.length}个`);
    }
    if (func && 'max' in func && args.length > func.max) {
      throw new Error(`参数个数错误：方法${func.value}，最多有${func.max}个参数，而不是${args.length}个`);
    }
    ...
    args.forEach(arg => {
      if (arg.type === 'fun') {
        if (arg.func.type === 'aggregator') {
          this.tokenStream.tokenRaiseError(`聚合粒度错误：方法${func.value},不能用于已聚合`);
        }
      }
    });
  }
