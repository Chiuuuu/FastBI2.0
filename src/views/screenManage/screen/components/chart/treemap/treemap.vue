...
export default {
  ...
  methods: {
    ...
    doWithOptions(fetchData, originDimensions, measures) {
      ...
      this.isRenderMeasure =
        echart.customPiecesIndex > originDimensions.length - 1

      // 如果是按度量分配颜色就从维度的最后开始
      const dimensions = cache.slice(
        this.isRenderMeasure
          ? originDimensions.length - 1
          : echart.customPiecesIndex
      )
      ...
      const showWay = {
        measure: () => {
          const dimensionIndex =
            originDimensions.length - echart.customPiecesIndex - 1
          visualMap = this.doWithVisualMap(dimensionIndex, pieces)
        },
        average: () => {
          const dimensionIndex =
            originDimensions.length - echart.customPiecesIndex
      ...
      if (!fn) { return console.error(`There is no method: [${echart.customShowWay}]`) }
      fn()
    ...
    getDefaultData() {
      ...
      this.treeRoot = ''
      let options = this.doWithOptions(
        defaultData,
    ...
    updateChartStyle() {
      ...
      let newOptions
      if (this.serverData) {
        newOptions = this.doWithOptions(
          this.serverData,
          this.options.data.dimensions,
          this.options.data.measures
        )
      } else {
        newOptions = this.doWithOptions(
          defaultData,
