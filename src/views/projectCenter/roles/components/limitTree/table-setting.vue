...
<script>
export default {
  inject: ['status', 'getCurrentRoleTab'],
  props: {
    visible: Boolean,
  },
  data() {
  ...
  computed: {
      ...
      return [
        {
          ...
          children: this.tableData.filter(
            item => item.dbName.toLowerCase().indexOf(this.tableSearchWord.toLowerCase()) > -1,
          ),
        },
      ];
    },
    ...
    searchTableKey() {
      if (this.treeData.length === 0) {
        ...
        return this.checkedTables.filter(item => this.treeData[0].children.some(t => t.dbId === item));
      }
    },
  },
  methods: {
    ...
    // 库组模态框-确认
    handleOkTable() {
      if (this.checkedTables.some(item => item.id === this.currentSourceInfo.id)) {
