<template>
  <div class="middle">
    ...
    <a-tabs class="tabs scrollbar" @change="handleChangeTab">
      <a-tab-pane forceRender :key="1" tab="数据大屏">
        <a-spin :spinning="spinning1">
          <role-limit v-on="$listeners" ref="module1" role-title="数据大屏" :options="modulePermission[1] || {}" />
        </a-spin>
      </a-tab-pane>
      <a-tab-pane forceRender :key="2" tab="数据模型">
        <a-spin :spinning="spinning2">
          <role-limit v-on="$listeners" ref="module2" role-title="数据模型" :options="modulePermission[2] || {}" />
        </a-spin>
      </a-tab-pane>
      <a-tab-pane forceRender :key="3" tab="数据接入">
        <a-spin :spinning="spinning3">
          <role-limit v-on="$listeners" ref="module3" role-title="数据接入" :options="modulePermission[3] || {}" />
        </a-spin>
      </a-tab-pane>
    </a-tabs>
  </div>
...
export default {
  ...
  methods: {
    ...
    handleGetSourceType() {
      this.$server.projectCenter.getRoleSourceType(this.$store.state.projectRoles.roleId).then(res => {
        if (res && res.code === 200) {
          let list = 'all';
        ...
        for (let i = 0; i < this.treeList[index].length; i++) {
          ...
          if (item.fileType === 0) {
            ...
            if (perm !== 'null' && !item.permissions.includes(perm)) {
              stop = true;
              break;
            } else if (item.children && item.children.length && item.children.length > 0) {
