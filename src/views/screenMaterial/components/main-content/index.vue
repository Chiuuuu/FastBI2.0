  ...
  methods: {
    ...
    handleImgMouseMove(e) {
      if (this.isMovingImage) {
        // 获取滚动窗
        const scrollBody = document.querySelector('.previewer.ant-modal-root .ant-modal-body');
        // 当前可滚动的距离
    ...
    handleSaveMaterial() {
      this.$refs.form.validate(async ok => {
        if (ok) {
          ...
          this.loading = true;
          const result = await this.$server.screenMaterial.addMaterial(formData).finally(() => {
            this.loading = false;
          });
          if (result.code === 200) {
