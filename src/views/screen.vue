<template>
  <div class="dv-screen" ref="dvScreen" :style="dvScreenStyle" @contextmenu.stop.prevent="poupExportMenu($event)">
...
export default {
  ...
  computed: {
    ...
    dvScreenStyle() {
      return this.isShootDom ? { width: `${this.pageSettings.width}px`, height: 'auto' } : null;
    },
    scrollBoxStyle() {
      return {
  ...
  methods: {
    ...
    getShareData() {
      ...
      return this.$server.screenManage.showScreenRelease(this.screenId).then(res => {
        if (res.code === 200) {
