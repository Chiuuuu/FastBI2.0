<template>
  <div class="flex-col">
    <div class="role-list-controller">
      ...
      <a-button class="btn-add" type="primary" ghost @click="showModal('add')" :disabled="loading">添加</a-button>
    </div>
    <a-table
    ...
    >
      ...
      <span slot="deptName">
        部门
        <a-icon class="edit-icon" type="setting" @click="handleSetDepart" />
      </span>
      <!-- 岗位 -->
      <span slot="postName">
        岗位
        <a-icon class="edit-icon" type="setting" @click="handleSetPost" />
      </span>
      <!-- 所属项目 -->
      <template #projects="text">
        <span :title="text ? text.toString() : ''">{{ text ? text.toString() : '' }}</span>
      </template>
      <!-- 是否启用 -->
      ...
      <template #config="text, record">
        <template v-if="record && record.id !== adminId">
          ...
          <a-popconfirm title="是否确定删除？" ok-text="确定" cancel-text="取消" @confirm="handleDelete(record.id)">
            <a href="#">删除</a>
          </a-popconfirm>
        </template>
      </template>
    ...
    <!-- 部门管理弹窗 -->
    <ModalListManager :options="deptOptions" @refresh="handleGetDepartList" @close="deptOptions.visible = false" />
...
export default {
  ...
  methods: {
    ...
    async handleGetDepartList() {
      const res = await this.$server.corporateDomain.getDeptList().finally(() => {
        this.spinning = false;
      });
      if (res.code === 200) {
    ...
    async handleGetPostList(id) {
      ...
      const res = await this.$server.corporateDomain.getPostList(id).finally(() => {
        this.spinning = false;
      });
      if (res.code === 200) {
    ...
    async handleGetData(pagination) {
      ...
      const res = await this.$server.corporateDomain.getUserListByParams(params).finally(() => {
        this.loading = false;
      });
