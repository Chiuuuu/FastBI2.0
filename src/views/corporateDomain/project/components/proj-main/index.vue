  ...
  <div class="main">
    <div class="right" style="padding-top: 16px">
      <div class="role-list-controller">
        <div class="searchbar">
          <a-form-model layout="inline" :model="searchForm">
            ...
            <a-form-model-item>
              <a-button type="default" @click="handleResetForm" :disabled="loading">重置</a-button>
            </a-form-model-item>
          </a-form-model>
        </div>
        <a-button class="btn-add" type="primary" ghost @click="showModal('add')" :disabled="loading">添加项目</a-button>
      </div>
      <a-table
      ...
      >
        ...
        <template #config="text, record, index">
          <a-popover placement="left" trigger="click">
            <template slot="content">
              <a-spin :spinning="getUserByProjectLoading" size="small">
                <div v-for="(item, index) in projectUsers" :key="index" :title="item.userName" class="pop-user-list">
          ...
          <a-popconfirm title="是否确定删除？" ok-text="确定" cancel-text="取消" @confirm="handleDelete(record, index)">
            <a href="#">删除</a>
          </a-popconfirm>
        </template>
      </a-table>
    ...
    >
      <a-form-model ref="form" :model="form" :rules="rules" :label-col="{ span: 6 }" :wrapper-col="{ span: 16 }">
        <a-form-model-item label="项目名称" prop="projectName">
          <a-input v-model="form.projectName" style="width: 100%" placeholder="请输入项目名称" />
        </a-form-model-item>
        <a-form-model-item label="项目描述" prop="projectDesc">
          <a-input v-model="form.projectDesc" style="width: 100%" placeholder="请输入项目描述" />
        </a-form-model-item>
        <a-form-model-item label="管理员" prop="adminList">
          ...
          <a-select
            ...
            placeholder="请选择管理员"
          >
            <a-select-option v-for="item in adminList" :key="item.id" :value="item.id" :disabled="item.id === '1'">
              {{ item.name }}
            </a-select-option>
          </a-select>
        </a-form-model-item>
