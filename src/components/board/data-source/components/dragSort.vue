    ...
    <div v-if="fileList.length > 0">
      ...
      >
        <a-dropdown :trigger="['click', 'contextmenu']" v-model="item.showMore">
          ...
          <a-menu slot="overlay">
            <a-sub-menu key="1" title="聚合方式" v-show="item.polymerizationShow">
              <a-menu-item
            ...
            <a-menu-item key="2" @click.native="changeAsc(item, 1)">升序</a-menu-item>
            <a-menu-item key="3" @click.native="changeAsc(item, 2)">降序</a-menu-item>
            <a-menu-item key="4" @click.native="deleteFile(item, index)">移除</a-menu-item>
          </a-menu>
        </a-dropdown>
        <a-tooltip :title="formatAggregator(item)">
          <span ref="itemName" class="field-text">
            <icon-font style="font-weight: bolder" :type="item.asc === 1 ? 'icon-paixu-5' : 'icon-paixu-3'" />
    ...
    currSelected: {
      handler(val) {
        if (val) {
          this.fileList = [];
          // 当前选中的图表数据筛选的数据
          if (val.setting.api_data.options && val.setting.api_data.options.sort) {
