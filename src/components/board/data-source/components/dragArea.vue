  ...
  methods: {
    ...
    async getData() {
      ...
      if (this.chartType === '1') {
        ...
        if (data.dimensions.some(item => item.status === 1) || data.measures.some(item => item.status === 1)) {
          return;
        }
      }
      // }
      ...
      if (res.code === 200) {
        ...
        if (this.currSelected.setting.chartType === 'v-treemap') {
          ...
          let _series = res.rows.map(item => [item[dim[0]], item[dim[1]], item[mea[0]]]);
        ...
        if (this.type === 'tableList') {
          ...
          if (this.chartType === '2') {
            ...
            // 环形图第二度量(指针值)
            if (this.currSelected.setting.chartType === 'v-ring' && apiData.measures[1]) {
