      ...
      >
        ...
        >
          ...
          <a-menu slot="overlay">
            ...
            <a-menu-item key="3" @click.native="deleteFile(item, index)">移除</a-menu-item>
          </a-menu>
        </a-dropdown>
...
export default {
    ...
    currSelected: {
      handler(val) {
        if (val) {
          // 当前选中的图表显示维度度量的数据
          this.fileList = [];
          if (this.type === 'dimensions' && val.setting.api_data[this.dimensionType]) {
            // 维度
        ...
        if (val) {
          if (this.type === 'dimensions' && this.fileList) {
            ...
            this.fileList.forEach(file => {
              let dimension = val.labelDimensions.find(item => item.alias === file.alias);
          ...
          if (this.type === 'measures' && this.fileList) {
            // 度量
            this.fileList.forEach(file => {
              let measure = val.labelMeasures.find(item => item.alias === file.alias);
  ...
  methods: {
    ...
    judgeDataType(dataType) {
      // 判断数值类型
      let isNum = dataType === 'BIGINT' || dataType === 'DECIMAL' || dataType === 'DOUBLE';
    ...
    deleteFile(item, index) {
      ...
      if (
        ...
        // 清空数据
        current.setting.config.series = current.setting.config.series.filter(item => item.type === 'map');
    ...
    async getData() {
      ...
      if (selected.setting.api_data.labelNormal) {
        dimensionList = [...dimensionList, ...selected.setting.api_data.labelNormal];
      }
      if (selected.setting.api_data.labelLatitude) {
        dimensionList = [...dimensionList, ...selected.setting.api_data.labelLatitude];
      }
      if (selected.setting.api_data.labelLongitude) {
        dimensionList = [...dimensionList, ...selected.setting.api_data.labelLongitude];
      }
      ...
      if ((!selected.datamodelId || selected.datamodelId === '0') && this.fileList.length > 0) {
        ...
        selected.setting.api_data.modelId = parseInt(this.fileList[0].screenTableId);
      ...
      if (this.labelType === 'area') {
        if (apiData.labelDimensions.length === 0 || apiData.labelMeasures.length === 0) {
          return;
        }
        if (
        ...
        // 经纬度两个维度一个度量
        if (apiData.labelDimensions.length < 2 || apiData.labelMeasures.length === 0) {
          return;
        }
      ...
      if (res.code === 200) {
        res.data.labelList = await handleReturnChartData(res.data.labelList, this.currSelected.setting);
        ...
        if (this.labelType === 'area') {
          ...
          // 一个度量对应一个series.data
          apiData.labelMeasures.forEach(measure => {
          ...
          for (let data of res.data.labelList) {
            ...
            try {
              // 获取位置信息
              let position = [data[apiData.labelLatitude[0].alias], data[apiData.labelLongitude[0].alias]];
              positionMsg = await reverseAddressResolution(position);
              datas.push({
                name: positionMsg.district,
                value: [parseFloat(position[0]), parseFloat(position[1])].concat(data[alias]),
                // 构造映射数据，给指标提示框内容显示
                [apiData.labelLatitude[0].alias]: data[apiData.labelLatitude[0].alias], // 经度
                [apiData.labelLongitude[0].alias]: data[apiData.labelLongitude[0].alias], // 维度
          ...
          config.series.push(
            Object.assign({}, dotSeries, {
              ...
              name: alias,
              pointShowList: ['地区名'],
              tooltipShowList: [apiData.labelLatitude[0].alias, apiData.labelLongitude[0].alias, alias],
            }),
          );
        }
        loadingInstance.close();
