        ...
        if (val) {
          if (this.type === 'dimensions' && this.fileList) {
            ...
            this.fileList.forEach(file => {
              let dimension = val.dimensions.find(item => item.alias === file.alias);
  ...
  methods: {
    ...
    deleteFile(item, index) {
      ...
      if (current.setting.api_data.dimensions.length === 0 && current.setting.api_data.measures.length === 0) {
        // 清空数据
        current.setting.config.series = current.setting.config.series.filter(item => item.type === 'scatter');
    ...
    async getData() {
      ...
      if (selected.setting.api_data.latitude) {
        dimensionList = [...dimensionList, ...selected.setting.api_data.latitude];
      }
      if (selected.setting.api_data.longitude) {
        dimensionList = [...dimensionList, ...selected.setting.api_data.longitude];
      }
      ...
      if (res.code === 200) {
        res.data.fillList = await handleReturnChartData(res.data.fillList, this.currSelected.setting);
        ...
        if (this.fillType === 'area') {
          ...
          config.series.unshift(
            Object.assign({}, mapSeries, {
              ...
              name: alias + 'fill',
              pointShowList: ['地区名'],
              tooltipShowList: [apiData.latitude[0].alias, apiData.longitude[0].alias, alias],
            }),
          );
          valueList = datas.map(item => item.value);
        }
