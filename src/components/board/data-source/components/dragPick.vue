    ...
    <a-modal v-model="screenVisible" title="数据筛选">
      ...
      <div v-if="currentFile.file === 'dimensions'">
        ...
        <input
          ...
          @input="isEmpty = false"
        />
        <button v-show="currentFile.operation === 'list'" class="ant-btn ant-btn-primary pick-btn" @click="search">
          查询
        </button>
        <input
          ...
          @input="isEmpty = false"
        />
        <button v-show="currentFile.operation === 'manual'" class="ant-btn ant-btn-primary" @click="addManualProperty">
          添加
        </button>
        ...
        <div class="pick-checkbox-box" style="margin: 0; padding: 0">
          <b-scrollbar style="height: 100%">
            ...
            <div :class="['pick-condition-box']" v-for="(item, index) in currentFile.conditionList" :key="index">
              <a-select :class="['pick-select', 'has-margin']" v-model="item.condition" @change="changeCondition(item)">
    ...
    currSelected: {
      handler(val) {
        if (val) {
          this.fileList = [];
          // 当前选中的图表数据筛选的数据
          if (val.setting.api_data.options && val.setting.api_data.options.fileList) {
  ...
  computed: {
    ...
    isNoSelectData() {
      if (
        ...
        ((this.currentFile.operation === 'list' && this.currentFile.checkedList.length === 0) ||
          (this.currentFile.operation === 'manual' && this.currentFile.manualList.length === 0))
      ) {
        return true;
      }
      return false;
  ...
  methods: {
    ...
    async handleDropOnFilesWD(event) {
      ...
      if (dataFile.file === 'dimensions') {
        ...
        if (dataType !== 'BIGINT' && dataType !== 'DECIMAL' && dataType !== 'DOUBLE') {
    ...
    search() {
      ...
      this.currentFile.searchList = this.currentFile.originList.filter(item => item.indexOf(this.listValue) > -1);
    },
    // 点击再次打开数据筛选弹窗
    async showSelect(item) {
      ...
      if (item.file === 'dimensions') {
        ...
        let res = await this.$server.screenManage.getDataPick(params);
        this.currentFile.originList = res.rows.map(item => Object.values(item)[0].toString()); // 维度全字段列表
        this.currentFile.searchList = this.currentFile.originList;
        this.onChange();
      }
      this.screenVisible = true;
    ...
    async handleOk() {
      ...
      // 度量没有添加条件不能确定
      if (this.currentFile.file === 'measures' && !this.currentFile.conditionList.length) {
        this.screenVisible = false;
        return;
      }
      ...
      if (this.currentFile.file === 'dimensions') {
        ...
        this.currentFile.value =
          this.currentFile.operation === 'list' ? this.currentFile.checkedList : this.currentFile.manualList;
        ...
        if (
          this.currentFile.conditionList.some(
            item => !item.firstValue || (item.condition === 'range' && !item.secondValue),
          )
        ) {
    ...
    async getData() {
      ...
      if (type === '1') {
        ...
        if (apiData.dimensions.some(item => item.status === 1) || apiData.measures.some(item => item.status === 1)) {
          return;
        }
      }
      if (type === '2') {
        ...
        if (this.currSelected.setting.chartType === 'v-ring' && apiData.measures.length < 2) {
          return;
        }
        ...
        if (this.currSelected.setting.chartType === 'v-tables') {
          ...
          if (this.chartType === '2') {
            ...
            if (this.currSelected.setting.chartType === 'v-ring') {
              ...
              config.title.text = +((rows[0].value / datas[0][keys[1]]) * 100).toFixed(2) + '%';
              this.$store.dispatch('SetSelfProperty', config);
            }
            apiData.source = {
              columns,
