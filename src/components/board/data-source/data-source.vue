  ...
  <div class="data-source">
    <a-collapse v-model="activeKey" :bordered="false">
      ...
      <a-collapse-panel
        ...
        "
      >
        <drag-area v-if="currSelected.setting.chartType !== 'v-ring'" type="measures" ref="child"></drag-area>
        <!-- 环形图 -->
        <dragAreaForRing
      ...
      <!-- 地图填充 -->
      <a-collapse-panel key="mapFill" header="填充" v-if="currSelected.setting.chartType === 'v-map'">
        <a-collapse v-model="activeKey" :bordered="false">
          <a-radio-group class="radio-box" v-model="fillType">
            <a-radio value="area" @click.native.stop="onRadioChange('area', 'fillType')">地区</a-radio>
            <a-radio value="dot" @click.native.stop="onRadioChange('dot', 'fillType')">经纬度</a-radio>
          </a-radio-group>
          <a-collapse-panel key="dimensions" header="维度">
            <dragAreaForMapFill v-if="fillType === 'area'" type="dimensions" :fillType="fillType"></dragAreaForMapFill>
            <dragAreaForMapFill
              v-if="fillType === 'dot'"
              type="dimensions"
              :fillType="fillType"
          ...
          <a-collapse-panel key="measures" header="度量">
            <dragAreaForMapFill type="measures" :fillType="fillType"></dragAreaForMapFill>
          </a-collapse-panel>
          <!-- 开关移到样式，不需要拖数据，暂时保留 -->
          <!-- <GuiField label="指标显示">
            <a-switch
      ...
      <!-- 地图标记点 -->
      <a-collapse-panel key="mapLabel" header="标记点" v-if="currSelected.setting.chartType === 'v-map'">
        <a-collapse v-model="activeKey" :bordered="false">
          <a-radio-group class="radio-box" v-model="labelType">
            <a-radio value="area" @click.native.stop="onRadioChange('area', 'labelType')">地区</a-radio>
            <a-radio value="dot" @click.native.stop="onRadioChange('dot', 'labelType')">经纬度</a-radio>
          </a-radio-group>
          <a-collapse-panel key="dimensions" header="维度">
            <dragAreaForMapLabel
              v-if="labelType === 'area'"
          ...
          <a-collapse-panel key="measures" header="度量">
            <dragAreaForMapLabel type="measures" :labelType="labelType"></dragAreaForMapLabel>
          </a-collapse-panel>
        </a-collapse>
        <!-- 开关移到样式，不需要拖数据，暂时保留 -->
        <!-- <GuiField label="指标显示">
      ...
      <a-collapse-panel
        ...
        header="数据筛选"
        v-if="
          (chartType === '1' || chartType === '2' || chartType === '3') && currSelected.setting.chartType !== 'v-map'
        "
      >
...
export default {
  ...
  computed: {
    ...
    hasScatter() {
      return this.selfConfig.series ? this.selfConfig.series.some(item => item.type === 'scatter') : false;
    },
  },
  methods: {
