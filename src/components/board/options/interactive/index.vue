      ...
      <div class="pick-checkbox-box">
        <b-scrollbar style="height: 100%">
          ...
          <a-checkbox-group class="f-flexcolumn" v-model="bindList" @change="checkSelectAll">
            <div class="f-flexcolumn" v-for="chart in chartList" :key="chart.id">
              <a-checkbox
                ...
                :value="chart.id"
                :disabled="checkHaveBind(chart) || checkBeBinded(chart) || chart.id === currentSelected"
              >
                {{ chart.setting.config.title.text || chart.setting.config.title.content }}
          ...
          <a-checkbox style="margin-top: 5px" default-checked disabled>
            {{ currSelected.setting.config.title.text || currSelected.setting.config.title.content }}（当前报表）
          </a-checkbox>
        </b-scrollbar>
      </div>
    </a-modal>
...
export default {
  ...
  computed: {
    ...mapGetters(['currentSelected', 'currSelected', 'canvasMap', 'selectedModelList']),
    // 拖入一个维度的进入联动选择列
    toBindList() {
      // 排除当前图表id
    ...
    currentData() {
      // 没有使用数据文件
      if (!this.currSelected.datamodelId || this.currSelected.datamodelId === '0') {
        return '';
      }
      // 找到当前使用的数据信息
      let obj = this.selectedModelList.find(item => item.tableId === this.currSelected.datamodelId);
