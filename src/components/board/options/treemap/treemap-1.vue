<template>
  ...
  <div>
    ...
    <gui-field label="矩形颜色">
      <a-select v-model="selfConfig.series.recDimensionIndex" style="width: 164px" @change="changeDimension">
        <a-select-option v-for="(item, index) in concatDimAndMea" :key="item.alias" :value="index">
          {{ item.alias }}
        </a-select-option>
      </a-select>
    </gui-field>
    <gui-field label="矩形大小">
      <a-select v-model="selfConfig.series.recSize" style="width: 164px" @change="changeRecSize">
