<template>
  <div class="dv-charts" style="width: 100%; height: 100%" :style="backgroundStyle" ref="wrap">
    ...
    >
      <span>{{ chartType === 'v-ring' ? config.topTitle.content : config.title.content }}</span>
    </div>
    <component
      :key="key"
...
export default {
  ...
  methods: {
      ...
      if (this.typeName === 've-scatter') {
        ...
        if (this.apiData.selectData) {
          ...
          seriesData.data = [
            {
              name: '',
              value: [rows1[columns[1]], rows1[columns[2]], rows1[columns[0]], columns[1], columns[2], columns[0]],
            },
          ];
    ...
    resetChartStyle() {
      ...
      if (series && series.itemStyle && series.itemStyle.normal && series.itemStyle.normal.color) {
    ...
    setMapFormatter() {
      for (let series of this.chartSeries) {
        // 指标内容
        let isShowAreaName = series.pointShowList.some(str => str.search('地区名') > -1);
    ...
    _calcStyle() {
      ...
      if ((this.config.title && this.config.title.show) || (this.config.topTitle && this.config.topTitle.show)) {
