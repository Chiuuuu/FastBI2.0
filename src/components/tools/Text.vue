<template>
  <div class="text-box" ref="textBox" :style="backgroundStyle">
    <div
      ...
      :class="['editor-text', { 'editor-text-placholder': isShowPlaceHolder }, { 'cursor-text': editable }]"
      :style="contentStyle"
      ref="editorText"
    ></div>
  </div>
...
export default {
  ...
  watch: {
    ...
    'apiData.refreshData'(val) {
      if (val && !this.editable) {
        this.$nextTick(() => {
          this.$refs.editorText.innerHTML = this.htmlText.replace(reg, (match, alias) => {
  ...
  methods: {
    ...
    async getContent(isInit) {
      ...
      if (this.apiData.measures.length > 0) {
        ...
        selected.setting.api_data.source = await handleReturnChartData(res.rows, selected.setting);
  ...
  computed: {
    ...mapGetters(['contextMenuInfo', 'currentSelected', 'currSelected', 'modelMeasures', 'canvasMap']),
    contentStyle() {
      return this.config
        ? {
            padding: '0 10px',
    ...
    isShowPlaceHolder() {
      return (
        ...
        (!this.htmlText || this.htmlText === '<p><br></p>' || this.htmlText === '<br>')
      );
    },
    contextmenus() {
      let noDataList = [
