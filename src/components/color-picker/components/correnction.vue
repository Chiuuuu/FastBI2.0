<template>
  <div class="color-correction-wrapper" v-show="visible">
    <!-- 颜色版 start -->
    <div class="color-panel" ref="js-color-panel" @mousedown.stop="handleGetPanel">
      <!-- 滑块 start -->
    ...
    <div class="bar-block">
      <!-- 单色 start -->
      <div class="color-line" ref="js-color-hue" @mousedown.stop="handleGetHue"></div>
      <!-- 单色 end -->
      <!-- 滑块 start -->
    ...
    <div class="bar-block transparency">
      <!-- 单色 start -->
      <div class="color-line" ref="js-color-alpha" @mousedown.stop="handleGetAlpha"></div>
      <!-- 单色 end -->
      <!-- 滑块 start -->
  ...
  methods: {
    ...
    initDomStyle() {
      ...
      this.setPanelSliderStyle({
        left: `${Math.round(s * this.rect.panel.width - this.rect.panelSlider.width)}px`,
    ...
    changePanelStyle(options) {
      ...
      const cacheH = this.colorPicker.model.cacheH ? this.colorPicker.model.cacheH : h;
    ...
    handleGetPanel(event) {
      ...
      const v = Math.round(
        ((this.rect.panel.top + this.rect.panel.height - event.clientY) / this.rect.panel.height) * 100,
      );
    ...
    panelSliderMousemove(mouse) {
      ...
      const s = Math.round((shiftX / this.rect.panel.width) * 100);
      const v = Math.round(
        ((this.rect.panel.top + this.rect.panel.height - mouse.clientY) / this.rect.panel.height) * 100,
      );
      this.changePanelStyle({
        shiftX,
        shiftY,
    ...
    changeHueStyle(options) {
      ...
      this.colorPicker.update(HSVAToRGB(hue, Math.round(s * 100), Math.round(v * 100), a));
    },
    /*
     * 点击获取色相板
     */
    ...
    changeAlphaStyle(options) {
      ...
      this.colorPicker.update(HSVAToRGB(h, Math.round(s * 100), Math.round(v * 100), alpha));
    },
    /*
     * 点击获取透明度
     */
