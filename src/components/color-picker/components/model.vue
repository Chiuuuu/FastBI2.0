    ...
    <div class="color-value-box">
      ...
      <div class="color-value-block" v-if="model.type === 'RGBA'">
        <div class="color-value-unit">
          <input :value="R" @change="handleChangeUnitValue($event, 'RGBA', 0)" />
          <div class="title">R</div>
        </div>
        <div class="color-value-unit">
          <input :value="G" @change="handleChangeUnitValue($event, 'RGBA', 1)" />
          <div class="title">G</div>
        </div>
        <div class="color-value-unit">
          <input :value="B" @change="handleChangeUnitValue($event, 'RGBA', 2)" />
          <div class="title">B</div>
        </div>
        <div class="color-value-unit">
          <input :value="A" @change="handleChangeUnitValue($event, 'RGBA', 3)" />
      ...
      <div class="color-value-block" v-if="model.type === 'HSLA'">
        <div class="color-value-unit">
          <input :value="H" @change="handleChangeUnitValue($event, 'HSLA', 0)" />
          <div class="title">H</div>
        </div>
        <div class="color-value-unit">
          <input :value="S" @change="handleChangeUnitValue($event, 'HSLA', 1)" />
          <div class="title">S</div>
        </div>
        <div class="color-value-unit">
          <input :value="L" @change="handleChangeUnitValue($event, 'HSLA', 2)" />
          <div class="title">L</div>
        </div>
        <div class="color-value-unit">
          <input :value="A" @change="handleChangeUnitValue($event, 'HSLA', 3)" />
          <div class="title">A</div>
        </div>
      </div>
      <!-- HSVA end -->
...
export default {
  ...
  methods: {
    ...
    handleChange(type) {
      ...
      let index = this.modelList.findIndex(model => model.type === this.model.type);
      index += count;
    ...
    handleChangeUnitValue(event, type, index) {
      if (type === 'RGBA') {
        [this.R, this.G, this.B, this.A] = [this.R, this.G, this.B, this.A].map((_val, _index) => {
          if (index === _index) {
      ...
      if (type === 'HSLA') {
        [this.H, this.S, this.L, this.A] = [this.H, this.S, this.L, this.A].map((_val, _index) => {
          if (index === _index) {
            _val = event.target.value.trim();
          }
          return _val;
