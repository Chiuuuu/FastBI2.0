export default class PositionCommand {
  ...
  moveup() {
    const item = this.receiver.components.splice(this.index + 1, 1, this.receiver.components[this.index]).pop();
  ...
  execute() {
    if (this.method === 'top' && this.index !== this.receiver.components.length - 1) {
    ...
    } else if (this.method === 'up' && this.index !== this.receiver.components.length - 1) {
      this.moveup();
