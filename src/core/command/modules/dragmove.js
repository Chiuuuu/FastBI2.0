...
export default class DragMoveCommand extends BaseCommand {
  ...
  executeFun() {
    this.receiver.setting.style = merge(this.receiver.setting.style, this.newStyle);
    this.setCurCom();
  }
  undoFun() {
    this.receiver.setting.style = merge(this.receiver.setting.style, this.oldStyle);
    this.setCurCom();
  }
}
