        ...
        if (res.code === 200) {
          ...
          rootGetters.canvasMap.forEach(chart => {
            ...
            if (apiData.interactive) {
              if (apiData.interactive.beBinded === rootGetters.currentSelected) {
                apiData.interactive.beBinded = '';
                updateList.push(chart);
              }
              apiData.interactive.bindedList = _.pull(apiData.interactive.bindedList, rootGetters.currentSelected);
            }
          });
    ...
    async updateChartData({ state, rootGetters }, id) {
      ...
      if (params.setting.api_data.selectData) {
        ...
        if (params.setting.name === 've-heatmap') {
          ...
          let _series = dataList.map(item => [item[dim[0]], item[dim[1]], item[mea[0]]]);
