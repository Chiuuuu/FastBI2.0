import { summary } from '@/utils/summaryList';
...
export async function handleRefreshData({ chart, newData }) {
  if (chart.setting.chartType === 'v-map') {
    ...
    if (newData.labelList) {
      newData.labelList = await handleReturnChartData(newData.labelList, chart.setting, true);
    }
  ...
  if (chart.setting.type === '2') {
    ...
    if (chart.setting.chartType === 'v-ring') {
      ...
      chart.setting.config.topTitle.text = +((rows[0].value / datas[keys[1]]) * 100).toFixed(2) + '%';
    }
    // 仪表盘
    if (chart.setting.chartType === 'v-gauge' && chart.setting.api_data.measures[1]) {
      chart.setting.config.series.max = newData[0][chart.setting.api_data.measures[1].alias];
    }
    // 文本框
    ...
    // 嵌套饼图设置apis
    let rows = [];
    let dimensionKeys = chart.setting.api_data.dimensions.map(item => item.alias);
    dimensionKeys.forEach(item => {
      // 根据当前维度分类得到的列表
      let list = summary(newData, item, chart.setting.api_data.measures[0].alias); // 嵌套饼图度量只有一个，直接取第一个数
  ...
  if (newData.fillList) {
    ...
    if (apiData.options.fillType === 'area') {
      ...
      seriesObj = Object.assign(mapSeries, {
        ...
        name: name + 'fill',
        pointShowList: ['地区名'],
        tooltipShowList: [apiData.latitude[0].alias, apiData.longitude[0].alias, name],
      });
      valueList = datas.map(item => item.value);
    }
    // 本来有填充层改数据，没有加上
    ...
    if (apiData.options.labelType === 'area') {
      ...
      for (let data of newData.labelList) {
        ...
        try {
          ...
          datas.push({
            name: positionMsg.district,
            value: [parseFloat(position[0]), parseFloat(position[1])].cancat(data[alias]),
            // 构造映射数据，给指标提示框内容显示
